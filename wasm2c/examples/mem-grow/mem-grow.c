...
static void w2c_mem0x2Dgrow_f1(w2c_mem0x2Dgrow*);

FUNC_TYPE_T(w2c_mem0x2Dgrow_t0) = "\x89\x3a\x3d\x2c\x8f\x4d\x7f\x6d\x6c\x9d\x62\x67\x29\xaf\x3d\x44\x39\x8e\xc3\xf3\xe8\x51\xc1\x99\xb9\xdd\x9f\xd5\x3d\x1f\xd3\xe4";
FUNC_TYPE_T(w2c_mem0x2Dgrow_t1) = "\x36\xa9\xe7\xf1\xc9\x5b\x82\xff\xb9\x97\x43\xe0\xc5\xc4\xce\x95\xd8\x3c\x9a\x43\x0a\xac\x59\xf8\x4e\xf3\xcb\xfa\xb6\x14\x50\x68";

static FILE* log_file = NULL;
size_t total_checks = 0;

void wasm2c_mem0x2Dgrow_file_open() {
  log_file = fopen("mem0x2Dgrow_log.txt", "w");
}
void wasm2c_mem0x2Dgrow_add_callee(FunctionNode** graph, const char* caller, const char* callee) {
  FunctionNode *fn;
...
void wasm2c_mem0x2Dgrow_parse_line(FunctionNode** graph, char* line) {
  ...
  *colon = '\0';  // Null-terminate the caller
  ...
  while (callee) {
    wasm2c_mem0x2Dgrow_add_callee(graph, caller, callee);
    callee = strtok(NULL, ",");
  }
}

void wasm2c_mem0x2Dgrow_free_graph(FunctionNode*graph) {
...
void wasm2c_mem0x2Dgrow_print_graph(FunctionNode* graph) {
...
bool wasm2c_mem0x2Dgrow_contains_edge(const char *caller, const char *callee) {
...
void wasm2c_mem0x2Dgrow_push_stack(Stack *stack, const char *caller, const char *prev_caller, void *key) {
...
StackNode *wasm2c_mem0x2Dgrow_pop_stack(Stack *stack) {
  if (!stack->top) {
    return NULL;
  }
...
void wasm2c_mem0x2Dgrow_free_stack_node(StackNode *node){
  if (!node) {
    return;
  }
  free(node);
...
void wasm2c_mem0x2Dgrow_destroy_stack(Stack *stack) {
  StackNode *current = stack->top;
  while (current) {
    StackNode *temp = current;
    current = current->next;
    wasm2c_mem0x2Dgrow_free_stack_node(temp);
  }
  free(stack);
}

void wasm2c_mem0x2Dgrow_print_stack(Stack *stack) {
  StackNode *current = stack->top;
  while (current) {
    current = current->next;
  }
...
void wasm2c_mem0x2Dgrow_mem_instrumentation(w2c_mem0x2Dgrow*instance, u64 var, const char *caller){
  void *ptr = (void*)((u64)w2c_mem0x2Dgrow_memory(instance) + (u64)var);
  MemoryInfo *existing = wasm2c_mem0x2Dgrow_map_find(ptr);
  total_checks++;
  if (existing) {
    // If the last verified is the current method or calls the current method
    if (existing->last_verified && strcmp(existing->last_verified, caller) == 0 || wasm2c_mem0x2Dgrow_contains_edge(existing->last_verified, caller)) {
    ...
    wasm2c_mem0x2Dgrow_map_insert(temp);
  }
  // Revalidate because there is no relationship in the call graph
  existing = wasm2c_mem0x2Dgrow_map_find(ptr);
  wasm2c_mem0x2Dgrow_push_stack(call_stack, existing->last_verified, caller, ptr);
  wasm2c_mem0x2Dgrow_print_stack(call_stack);
}
...
static void init_memories(w2c_mem0x2Dgrow* instance) {
  wasm_rt_allocate_memory(&instance->w2c_M0, 1, 2, 0);
}

static void init_instance_import(w2c_mem0x2Dgrow* instance, struct w2c_console* w2c_console_instance) {
  instance->w2c_console_instance = w2c_console_instance;
}

void wasm2c_mem0x2Dgrow_instantiate(w2c_mem0x2Dgrow* instance, struct w2c_console* w2c_console_instance) {
  assert(wasm_rt_is_initialized());
  init_instance_import(instance, w2c_console_instance);
...
void wasm2c_mem0x2Dgrow_free(w2c_mem0x2Dgrow* instance) {
...
void w2c_mem0x2Dgrow_f1(w2c_mem0x2Dgrow* instance) {
  FUNC_PROLOGUE;
  u32 var_i0;
  var_i0 = 1u;
  ...
  wasm_rt_segue_write_base(instance->w2c_M0.data);
#endif
  w2c_console_log(instance->w2c_console_instance, var_i0);
  FUNC_EPILOGUE;
  while (call_stack->top && strcmp(call_stack->top->caller, __func__) == 0){
    StackNode *temp = wasm2c_mem0x2Dgrow_pop_stack(call_stack);
    MemoryInfo *associated_info = wasm2c_mem0x2Dgrow_map_find(temp->key);
    if (associated_info){
