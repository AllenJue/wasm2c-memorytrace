...
static void w2c_loop_loop_0(w2c_loop*);
...
FUNC_TYPE_T(w2c_loop_t1) = "\x26\x10\x81\xe2\x21\x43\xd6\x01\x3e\x2d\x2f\x16\x17\x78\x6f\xba\xb3\x2f\x4d\x54\x9b\x8a\xa9\xdd\xbf\x53\x92\x3c\xd3\x71\xc6\xb2";
FUNC_TYPE_T(w2c_loop_t2) = "\x36\xa9\xe7\xf1\xc9\x5b\x82\xff\xb9\x97\x43\xe0\xc5\xc4\xce\x95\xd8\x3c\x9a\x43\x0a\xac\x59\xf8\x4e\xf3\xcb\xfa\xb6\x14\x50\x68";

static FILE* log_file = NULL;

void wasm2c_loop_file_open() {
  log_file = fopen("loop_log.txt", "w");
}
void wasm2c_loop_load_instrumentation(w2c_loop*instance, uint32_t var){
  ptr = (void*)((u64)instance->w2c_host_mem + (u64)var);
  printf("L: %p\n", ptr);
  fprintf(log_file, "L: %p\n", ptr);
  existing = wasm2c_loop_map_find(ptr);
  if (existing) {
    printf("Existing!\n");
    if (!existing->dirty ) {
    ...
    wasm2c_loop_map_insert(ptr, &temp);
  }
}

void wasm2c_loop_store_instrumentation(w2c_loop*instance, uint32_t var){
  ptr = (void*)((u64)instance->w2c_host_mem + (u64)var);
  printf("S: %p\n", ptr);
...
/* export: 'loop' */
void w2c_loop_loop(w2c_loop* instance) {
#if WASM_RT_USE_SEGUE
#if !WASM_RT_SEGUE_FREE_SEGMENT
...
#endif
  w2c_loop_loop_0(instance);
#if WASM_RT_USE_SEGUE && !WASM_RT_SEGUE_FREE_SEGMENT
  wasm_rt_segue_write_base(segue_saved_base);
#endif
}

static void init_instance_import(w2c_loop* instance, struct w2c_host* w2c_host_instance) {
  instance->w2c_host_instance = w2c_host_instance;
  instance->w2c_host_mem = w2c_host_mem(w2c_host_instance);
}

const u64 wasm2c_loop_min_host_mem = 1;
const u64 wasm2c_loop_max_host_mem = 65536;
const u8 wasm2c_loop_is64_host_mem = 0;

void wasm2c_loop_instantiate(w2c_loop* instance, struct w2c_host* w2c_host_instance) {
